    ...
    <v-container>
        <v-card width="1080" min-height="580" class="mx-auto pa-2" flat>
            ...
            <v-card class="mx-2" min-height="400" v-if="search.length != 0">
                ...
                <div v-else>
                    <v-card-text>
                        <v-alert type="error" class="d-flex">
                             <span> Não foram encontrados resultados.</span>
                             <v-btn icon dark small @click="cleanResults"> <v-icon>mdi-close-circle</v-icon> </v-btn>
                        </v-alert>
                    </v-card-text>
                </div>
            </v-card>
            <v-card flat v-else>
                <v-card-text>
                    <span>Faça sua busca na legislação  </span>
                </v-card-text>
            </v-card>
        </v-card>
    </v-container>
...
<script>
  ...
  export default {
      ...
      methods:{
          ...
          errorSize(){
            this.$store.dispatch("snackbars/setSnackbars", {text:`Mais de 50 resultados, refine sua busca!`, color:'success', timeout:'3000'})
          },
          cleanResults(){
              this.search = ''
              this.searchResults = []
